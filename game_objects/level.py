...
class Level:
    ...
    def _draw_layout(self):
        ...
        # --- TEXTURA CENTRAL (por ejemplo, brújula) ---
        compass_cfg = self.data.get("puzzle_config", {}).get("properties", {}).get("compass_floor")
        if compass_cfg:
            tex_id = self.texture_manager.get_texture(compass_cfg.get("texture_id", "tex_compass_rose"))
            if tex_id != -1:
                glEnable(GL_TEXTURE_2D)
                glBindTexture(GL_TEXTURE_2D, tex_id)
                glColor3f(1.0, 1.0, 1.0)

                pos = compass_cfg.get("position", [32.0, 0.01, 0.0])
                size = compass_cfg.get("size", [10.0, 10.0])
